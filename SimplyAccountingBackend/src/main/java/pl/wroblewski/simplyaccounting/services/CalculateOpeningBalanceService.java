...
public class CalculateOpeningBalanceService {
    ...
    public void calculateClosingBalance(Integer accountId, Integer cooperativeId, Short year,
        ...
        var openingBalance = openingBalanceService.getOpeningBalanceByAccountAndYear(accountId, year,
                cooperativeId);

        var endingBalance = OpeningBalanceDto.builder()
                .accountId(accountId)
                .year((short) (year + (short) 1))
                .balance(calculateBalance(openingBalance.getBalance(), debits, credits))
                .build();

        openingBalanceService.createEditOpeningBalance(endingBalance);
    }
